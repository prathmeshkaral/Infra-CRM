...
# Get subnet details and filter out unsupported AZs
data "aws_subnet" "filtered" {
  count = length(data.aws_subnets.default.ids)
  id    = element(data.aws_subnets.default.ids, count.index)
}
...
resource "aws_eks_cluster" "cbz_cluster" {
  ...
  vpc_config {
    subnet_ids = [
      for s in data.aws_subnet.filtered : s.id
...
resource "aws_eks_node_group" "cbz_nodegroup" {
  ...
  # Ensure only supported AZs are used
  subnet_ids = [
    for s in data.aws_subnet.filtered : s.id
    if contains(["us-east-1a", "us-east-1b", "us-east-1c", "us-east-1d", "us-east-1f"], s.availability_zone)
  ]
